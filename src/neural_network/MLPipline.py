    ...
    for example, example_spect_labels in train_ds.take(1):
      ...
      for epochs in range(SUB_EPOCHS, EPOCHS + SUB_EPOCHS, SUB_EPOCHS):
        ...
        if result is not None:
          ...
          if save_model:
            target_path = os.path.join(model_exporter.export_path, 'epoch_' + str(epochs), 'model')
          ...
          if epochs == EPOCHS:
            model_exporter.export_evaluation_report(model_exporter.get_max_evaluation('acc'), self.af_type, self.model_type, 'acc')
            model_exporter.export_evaluation_report(model_exporter.get_max_evaluation('loss', aggregation='min'), self.af_type, self.model_type, 'loss')
            model_exporter.export_evaluation_report(model_exporter.get_max_evaluation('record_acc'), self.af_type, self.model_type, 'record_acc')
