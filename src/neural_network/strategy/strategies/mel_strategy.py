...
class MelStrategy(IAFStrategy):
  ...
  def _get_image_path(self, label: str):
    file_name = f"mel_{uuid.uuid4()}.png"
    directory = self.files.join(ASSETS_PATH, '__af__', AFTypes.mel.value, label)
    self.files.create_folder(directory)
    return os.path.join(directory, file_name)

  def get_audio_feature(self, wave: np.ndarray):
    melspectogram = self.features.melspectogram(signal=wave, sr=self.sr, frame_length=self.frame_length, hop_length=self.hop_length)
