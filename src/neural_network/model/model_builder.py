...
class ModelBuilder:
  ...
  def _get_latest_epoch(self, latest_checkpoint):
    return int(latest_checkpoint.split('.')[0]) if latest_checkpoint is not None else 1

  def is_need_to_train_for_epoch(self, epochs, latest_checkpoint = None, latest_epoch = None):
    latest_checkpoint = latest_checkpoint if latest_checkpoint is not None else self._get_latest_checkpoint()
    latest_epoch = latest_epoch if latest_epoch is not None else self._get_latest_epoch(latest_checkpoint)

    return epochs > latest_epoch
  ...
  def train(self, train_ds, val_ds, epochs):
    ...
    initial_epoch = self._get_latest_epoch(latest_checkpoint)

    if self.is_need_to_train_for_epoch(epochs, latest_checkpoint, initial_epoch) is False:
      return None
